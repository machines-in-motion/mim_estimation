...
class EKF:
    ...
    def __init__(self, conf):
        ...
        self.__nx = (
            5 * 3
        )  # delta_x = [delta_r, delta_v, delta_theta, delta_b_a, delta_b_omega]
    ...
    def compute_end_effectors_FK_quantities(
        ...
        for key, frame_name in self.__end_effectors_frame_names.items():
            ...
            end_effectors_velocities[
                key
            ] = frame_velocity.linear  # only linear velocity part
    ...
    def integrate_model(self, a_tilde, omega_tilde):
        ...
        b_a_post, b_omega_post = (
            self.__mu_post["bias_acceleration"],
            self.__mu_post["bias_orientation"],
        )
        ...
        q_pre.normalize()
        self.__mu_pre["base_position"] = (
            self.__mu_post["base_position"] + v_post * dt
        )
        self.__mu_pre["base_velocity"] = (
            self.__mu_post["base_velocity"]
            + (-pin.skew(omega_hat) @ v_post + R_post.T @ g + a_hat) * dt
        )
        self.__mu_pre["base_orientation"] = q_pre
        self.__mu_pre["bias_acceleration"] = self.__mu_post[
            "bias_acceleration"
        ]
    ...
    def update_step(
        ...
        self.__mu_post["base_position"] = (
            self.__mu_pre["base_position"] + delta_x[0:3]
        )
        self.__mu_post["base_velocity"] = (
            self.__mu_pre["base_velocity"] + delta_x[3:6]
        )
        self.__mu_post["base_orientation"] = Quaternion(
            box_plus(R_pre, delta_x[6:9])
        )
        self.__mu_post["bias_acceleration"] = (
            self.__mu_pre["bias_acceleration"] + delta_x[9:12]
        )
        self.__mu_post["bias_orientation"] = (
            self.__mu_pre["bias_orientation"] + delta_x[12:15]
        )
