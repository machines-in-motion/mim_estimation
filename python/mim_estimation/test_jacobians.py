...
if __name__ == "__main__":
    ...
    J_num_prediction[0:3, 6] = (
        box_plus(R, theta_plus_dx) @ v - f_x["base_position"]
    ) / delta
    J_num_prediction[0:3, 7] = (
        box_plus(R, theta_plus_dy) @ v - f_x["base_position"]
    ) / delta
    J_num_prediction[0:3, 8] = (
        box_plus(R, theta_plus_dz) @ v - f_x["base_position"]
    ) / delta
    # base velocity-base velocity
    J_num_prediction[3:6, 3] = (
        -pin.skew(omega_hat) @ v_plus_dx
        + R.T @ g
        + a_hat
        - f_x["base_velocity"]
    ) / delta
    J_num_prediction[3:6, 4] = (
        -pin.skew(omega_hat) @ v_plus_dy
        + R.T @ g
        + a_hat
        - f_x["base_velocity"]
    ) / delta
    J_num_prediction[3:6, 5] = (
        -pin.skew(omega_hat) @ v_plus_dz
        + R.T @ g
        + a_hat
        - f_x["base_velocity"]
    ) / delta
    # base velocity-base orientation
    J_num_prediction[3:6, 6] = (
        -pin.skew(omega_hat) @ v
        + box_plus(R, theta_plus_dx).T @ g
        + a_hat
        - f_x["base_velocity"]
    ) / delta
    J_num_prediction[3:6, 7] = (
        -pin.skew(omega_hat) @ v
        + box_plus(R, theta_plus_dy).T @ g
        + a_hat
        - f_x["base_velocity"]
    ) / delta
    J_num_prediction[3:6, 8] = (
        -pin.skew(omega_hat) @ v
        + box_plus(R, theta_plus_dz).T @ g
        + a_hat
        - f_x["base_velocity"]
    ) / delta
    # base velocity-bias acceleration
    J_num_prediction[3:6, 9] = (
        -pin.skew(omega_hat) @ v
        + R.T @ g
        + a_hat_plus_dx
        - f_x["base_velocity"]
    ) / delta
    J_num_prediction[3:6, 10] = (
        -pin.skew(omega_hat) @ v
        + R.T @ g
        + a_hat_plus_dy
        - f_x["base_velocity"]
    ) / delta
    J_num_prediction[3:6, 11] = (
        -pin.skew(omega_hat) @ v
        + R.T @ g
        + a_hat_plus_dz
        - f_x["base_velocity"]
    ...
    J_num_prediction[3:6, 12] = (
        -pin.skew(omega_hat_plus_dx) @ v
        + R.T @ g
        + a_hat
        - f_x["base_velocity"]
    ) / delta
    J_num_prediction[3:6, 13] = (
        -pin.skew(omega_hat_plus_dy) @ v
        + R.T @ g
        + a_hat
        - f_x["base_velocity"]
    ) / delta
    J_num_prediction[3:6, 14] = (
        -pin.skew(omega_hat_plus_dz) @ v
        + R.T @ g
        + a_hat
        - f_x["base_velocity"]
    ) / delta
    # base orientation - base orienation
    J_num_prediction[6:9, 6] = (
        -pin.exp(omega_hat)
        @ (box_minus(box_plus(R, theta_plus_dx), R))
        / delta
    )
    J_num_prediction[6:9, 7] = (
        -pin.exp(omega_hat)
        @ (box_minus(box_plus(R, theta_plus_dy), R))
        / delta
    )
    J_num_prediction[6:9, 8] = (
        -pin.exp(omega_hat)
        @ (box_minus(box_plus(R, theta_plus_dz), R))
        / delta
    )
    ...
    J_num_meas[0:3, 3] = (
        -(v1 + delta * np.array([1, 0, 0]))
        - (pin.skew(omega_hat) @ R_pre @ p1)
        - f1
    ) / delta
    J_num_meas[0:3, 4] = (
        -(v1 + delta * np.array([0, 1, 0]))
        - (pin.skew(omega_hat) @ R_pre @ p1)
        - f1
    ) / delta
    J_num_meas[0:3, 5] = (
        -(v1 + delta * np.array([0, 0, 1]))
        - (pin.skew(omega_hat) @ R_pre @ p1)
        - f1
    ) / delta
    # base velocity from foot1-bias orientation
    J_num_meas[0:3, 12] = (
        -v1 - (pin.skew(omega_hat_plus_dx) @ R_pre @ p1) - f1
    ) / delta
    J_num_meas[0:3, 13] = (
        -v1 - (pin.skew(omega_hat_plus_dy) @ R_pre @ p1) - f1
    ) / delta
    J_num_meas[0:3, 14] = (
        -v1 - (pin.skew(omega_hat_plus_dz) @ R_pre @ p1) - f1
    ) / delta

    # base velocity from foot2-base velocity
    J_num_meas[3:6, 3] = (
        -(v2 + delta * np.array([1, 0, 0]))
        - (pin.skew(omega_hat) @ R_pre @ p2)
        - f2
    ) / delta
    J_num_meas[3:6, 4] = (
        -(v2 + delta * np.array([0, 1, 0]))
        - (pin.skew(omega_hat) @ R_pre @ p2)
        - f2
    ) / delta
    J_num_meas[3:6, 5] = (
        -(v2 + delta * np.array([0, 0, 1]))
        - (pin.skew(omega_hat) @ R_pre @ p2)
        - f2
    ) / delta
    # base velocity from foot2-bias orientation
    J_num_meas[3:6, 12] = (
        -v2 - (pin.skew(omega_hat_plus_dx) @ R_pre @ p2) - f2
    ) / delta
    J_num_meas[3:6, 13] = (
        -v2 - (pin.skew(omega_hat_plus_dy) @ R_pre @ p2) - f2
    ) / delta
    J_num_meas[3:6, 14] = (
        -v2 - (pin.skew(omega_hat_plus_dz) @ R_pre @ p2) - f2
    ) / delta

    # base velocity from foot3-base velocity
    J_num_meas[6:9, 3] = (
        -(v3 + delta * np.array([1, 0, 0]))
        - (pin.skew(omega_hat) @ R_pre @ p3)
        - f3
    ) / delta
    J_num_meas[6:9, 4] = (
        -(v3 + delta * np.array([0, 1, 0]))
        - (pin.skew(omega_hat) @ R_pre @ p3)
        - f3
    ) / delta
    J_num_meas[6:9, 5] = (
        -(v3 + delta * np.array([0, 0, 1]))
        - (pin.skew(omega_hat) @ R_pre @ p3)
        - f3
    ) / delta
    # base velocity from foot3-bias orientation
    J_num_meas[6:9, 12] = (
        -v3 - (pin.skew(omega_hat_plus_dx) @ R_pre @ p3) - f3
    ) / delta
    J_num_meas[6:9, 13] = (
        -v3 - (pin.skew(omega_hat_plus_dy) @ R_pre @ p3) - f3
    ) / delta
    J_num_meas[6:9, 14] = (
        -v3 - (pin.skew(omega_hat_plus_dz) @ R_pre @ p3) - f3
    ) / delta

    # base velocity from foot4-base velocity
    J_num_meas[9:12, 3] = (
        -(v4 + delta * np.array([1, 0, 0]))
        - (pin.skew(omega_hat) @ R_pre @ p4)
        - f4
    ) / delta
    J_num_meas[9:12, 4] = (
        -(v4 + delta * np.array([0, 1, 0]))
        - (pin.skew(omega_hat) @ R_pre @ p4)
        - f4
    ) / delta
    J_num_meas[9:12, 5] = (
        -(v4 + delta * np.array([0, 0, 1]))
        - (pin.skew(omega_hat) @ R_pre @ p4)
        - f4
    ) / delta
    # base velocity from foot4-bias orientation
    J_num_meas[9:12, 12] = (
        -v4 - (pin.skew(omega_hat_plus_dx) @ R_pre @ p4) - f4
    ) / delta
    J_num_meas[9:12, 13] = (
        -v4 - (pin.skew(omega_hat_plus_dy) @ R_pre @ p4) - f4
    ) / delta
    J_num_meas[9:12, 14] = (
        -v4 - (pin.skew(omega_hat_plus_dz) @ R_pre @ p4) - f4
    ) / delta
    ...
    print(
        np.testing.assert_allclose(
            J_num_prediction, J_anal, atol=np.sqrt(precision)
        )
    )
