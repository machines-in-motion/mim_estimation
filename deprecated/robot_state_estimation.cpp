...
#ifdef HAS_VICON
        ,athena_vicon_collect_(robot_math::RtThreadInfo("vicon_base_state_estimator_athena_vicon_collect", 0, 0))
        ,vicon_bse_(filtered_kinematics->getUpdatePeriod(), vis_tools_interface, n["vicon_bse"])
#endif
  {
    ...
    joint_sensors_filter_cutoff_ = n["joint_sensors_filter_cutoff"].as<double>();
    wrench_filter_cutoff_ = n["wrench_filter_cutoff"].as<double>();

  force_threshold_for_contact_ = n["force_threshold_for_contact"].as<double>();
}

void RobotStateEstimation::initialize(const ContactDescritpion (&contact_description)[Robot::n_endeffs_])
{
#ifdef HAS_VICON
  ...
  unfiltered_joint_position_sensor_->data().positions().segment<3>(Robot::dof_l_ar_-1) = Eigen::Vector3d(0., 0.329, 0.);
  unfiltered_joint_position_sensor_->data().positions().segment<3>(Robot::dof_r_ar_-1) = Eigen::Vector3d(0., 0.329, 0.);
  unfiltered_joint_position_sensor_->data().velocities().segment<3>(Robot::dof_l_ar_-1) = Eigen::Vector3d(0., 0., 0.);
  unfiltered_joint_position_sensor_->data().velocities().segment<3>(Robot::dof_r_ar_-1) = Eigen::Vector3d(0., 0., 0.);
...
//  const_cast<geometry_utils::Quaternion&>(simulated_base_state_->data().orient()).set(1.,0.,0.,0.);
  ...
  if(simulated_base_state_ != nullptr){
    ...
    if (use_vicon_base_) {
      ...
      vicon_kinematics::AthenaViconCollect::PostureVector posture_vec = joint_sensors_data.positions();
      bool new_frame = athena_vicon_collect_.get_base_measurement(posture_vec, base_pose_vicon);

      std::cout << "Using vicon..." << std::endl;
      vicon_bse_.initialize(unfiltered_joint_position_sensor_->data(),
                            base_pose_vicon.f_,
                            new_frame);
      Eigen::Vector3d vicon_bse_base_vel, vicon_bse_accel_bias, vicon_bse_gyro_bias;
  ...
  position_filter_.initialize(unfiltered_joint_position_sensor_->data().positions(), joint_sensors_filter_cutoff_);
  velocity_filter_.initialize(unfiltered_joint_position_sensor_->data().velocities(), joint_sensors_filter_cutoff_);
  for(int i=0; i<wrench_filters_.size(); ++i)
  {
    assert(i < unfiltered_wrench_sensors_.size());
    wrench_filters_[i].initialize(unfiltered_wrench_sensors_[i]->data().data(), wrench_filter_cutoff_);
  }
...
void RobotStateEstimation::update(){
  ...
  position_filter_.update(unfiltered_joint_position_sensor_->data().positions());
  velocity_filter_.update(unfiltered_joint_position_sensor_->data().velocities());
  ...
  for(int i=0; i<Robot::n_endeffs_; ++i)
  {
    ...
    filtered_wrench_[i].point_of_action() = unfiltered_wrench_sensors_[i]->data().point_of_action();
  }

  // compute and draw the foot and overall cops using knowledge of the floor height:
  //computeFootCoPsOnFloor();

  // Draw the projected foot contact points on the floor:
  int i = 0;
  for(auto eff_id:{EndeffId::right(),EndeffId::left()}) {
    filt_contact_points_.at(i) = getFootContactPointProjected(EndeffItr(eff_id) + 1);
    ++i;
  }
  vis_tools_interface_->drawBalls(filt_contact_points_, Eigen::Vector3d(1.0, 0.0, 0.0), 1.0, 0.02);
  ...
  if(simulated_base_state_ != nullptr){
    ...
    filtered_velocity_.base_linear_velocity() = simulated_base_state_->data().vel();
    filtered_posture_.base_orientation_ = simulated_base_state_->data().orient();
    filtered_velocity_.base_angular_velocity() = simulated_base_state_->data().ang_vel();
  }else{
#ifdef HAS_VICON
    if(use_vicon_base_){
  ...
  filtered_kinematics_->update(filtered_posture_, filtered_velocity_, dummy_acc, contacts_);


  for(int i=0; i<Robot::n_endeffs_; ++i)
  {
    endeff_wrench_[i] = filtered_wrench_[i].express_wrench_at(
        filtered_kinematics_->link_pose(unfiltered_wrench_sensors_[i]->link_id()),
...
void RobotStateEstimation::computeFootCoPsOnFloor() {
  ...
  // Compute CoPs assuming flat feet contacts with the ground (specific to Athena!):
  ...
  for(auto eff_id:{EndeffId::right(),EndeffId::left()}) {
    ...
    ft_sensor_tf = filtered_kinematics_->link_pose(unfiltered_wrench_sensors_[EndeffItr(eff_id)]->link_id());
    endeff_frc_world_frame = ft_sensor_tf.topLeftCorner(3,3) * filtered_wrench_[EndeffItr(eff_id)].force();
    endeff_trq_world_frame = ft_sensor_tf.topLeftCorner(3,3) * filtered_wrench_[EndeffItr(eff_id)].torque();
    ...
    filt_foot_cops_.at(i).segment(0,2) =
            robot_estimation::ContactHelper::center_of_pressure(
                    ...
                    floor_tf_, cop_torque);
    filt_foot_cops_.at(i) = geometry_utils::Transformations::transformVector(floor_tf_, filt_foot_cops_.at(i));
    filt_foot_cops_rgb_.at(i)(i) = 1.0;
    ++i;
  }

  // compute the overall CoP by transforming the right FT sensor force/torque to the left FT sensor location:
  Eigen::Matrix<double,6,1> total_wrench_left_FT = filtered_wrench_[Robot::id_left_foot_-1].wrench();
  Eigen::Matrix4d right_FT_to_left_FT_tf =
          geometry_utils::Transformations::invertHomogeneousTransform(
              filtered_kinematics_->link_pose(unfiltered_wrench_sensors_[Robot::id_left_foot_-1]->link_id())) *
          filtered_kinematics_->link_pose(unfiltered_wrench_sensors_[Robot::id_right_foot_-1]->link_id());
  total_wrench_left_FT += geometry_utils::Transformations::transformWrench(right_FT_to_left_FT_tf,
                                                                           filtered_wrench_[Robot::id_right_foot_-1].wrench());
  Eigen::Matrix<double,6,1> total_wrench_world = geometry_utils::Transformations::rotateWrench(
          filtered_kinematics_->link_pose(unfiltered_wrench_sensors_[Robot::id_left_foot_-1]->link_id()),
  ...
  filt_overall_cop_.segment(0,2) = robot_estimation::ContactHelper::center_of_pressure(
      ...
      geometry_utils::Transformations::transformVector(ft_sensor_tf,
                               ...
                               floor_tf_, cop_torque);
  filt_foot_cops_.at(2) = geometry_utils::Transformations::transformVector(floor_tf_, filt_overall_cop_);
  filt_foot_cops_rgb_.at(2)(2) = 1.0;

  vis_tools_interface_->drawBalls(filt_foot_cops_, filt_foot_cops_rgb_, 1.0, 0.03);
...
void RobotStateEstimation::update_vicon_base_state_ekf(){
  ...
  vicon_kinematics::AthenaViconCollect::PostureVector posture_vec = joint_sensors_data.positions();
  bool new_frame = athena_vicon_collect_.get_base_measurement(posture_vec, base_pose_vicon);
  vicon_bse_.update(unfiltered_imu_->data(), unfiltered_joint_position_sensor_->data(), base_pose_vicon.f_, new_frame);
  Eigen::Vector3d vicon_bse_base_vel, vicon_bse_accel_bias, vicon_bse_gyro_bias;
  ...
  filtered_velocity_.base_angular_velocity() = unfiltered_imu_->data().gyroscope() - vicon_bse_gyro_bias;
#endif
}
...
void RobotStateEstimation::update_base_estimation_ekf(){
  ...
  left_orien.rotation_matrix_to_quaternion(unfiltered_forward_kinematics_->endeffector_pose(Robot::id_left_foot_).topLeftCorner<3,3>());
  right_orien.rotation_matrix_to_quaternion(unfiltered_forward_kinematics_->endeffector_pose(Robot::id_right_foot_).topLeftCorner<3,3>());
  base_state_estimator_.update(unfiltered_imu_->data().accelerometer(),
                               unfiltered_imu_->data().gyroscope(),
                               unfiltered_forward_kinematics_->endeffector_position(Robot::id_right_foot_),
                               unfiltered_forward_kinematics_->endeffector_position(Robot::id_left_foot_),
                               right_orien,
                               left_orien,
                               contacts_[Robot::id_left_foot_].activation_(0), contacts_[Robot::id_right_foot_].activation_(0));
...
void RobotStateEstimation::subscribe_to_data_collector(data_collection::DataCollector& data_collector, std::string name)
{
#ifdef HAS_VICON
  if (use_vicon_base_) {
  ...
  data_collector.addVector(filtered_posture_.joint_positions_, name + std::string("_jfilt"), jnames, units);
  data_collector.addVector(filtered_velocity_.joint_velocities(), name + std::string("_djfilt"), jnames, units);

  data_collector.addVector(unfiltered_joint_position_sensor_->data().positions(), name + std::string("_jraw"), jnames, units);
  data_collector.addVector(unfiltered_joint_position_sensor_->data().velocities(), name + std::string("_djraw"), jnames, units);
  data_collector.addVector3d(unfiltered_imu_->data().gyroscope(), name + "_imu_base_gyro" , units[0]);
  data_collector.addVector3d(unfiltered_imu_->data().accelerometer(), name + "_imu_base_acc", units[0]);

  assert(EndeffItr(EndeffId::left()) < filtered_wrench_.size());
  assert(EndeffItr(EndeffId::right()) < filtered_wrench_.size());
  data_collector.addVector6d(endeff_wrench_[Robot::id_left_foot_-1].data(), name + "_LF_FT", "-");
  data_collector.addVector6d(endeff_wrench_[Robot::id_right_foot_-1].data(), name + "_RF_FT", "-");

  data_collector.addVector3d(filtered_posture_.base_position_,name + "_base_pos","-");
  data_collector.addQuaternion(filtered_posture_.base_orientation_.get() ,name + "_base_orient");
  data_collector.addVector3d(filtered_velocity_.base_linear_velocity(),name + "_base_lvel","-");
  data_collector.addVector3d(filtered_velocity_.base_angular_velocity(),name + "_base_avel","-");

  if(simulated_base_state_ != nullptr){
    data_collector.addVector3d(simulated_base_state_->data().pos(), name + "_sim_base_pos", "m");
    data_collector.addQuaternion(simulated_base_state_->data().orient().get(), name + "_sim_base_orient");
  }
}
