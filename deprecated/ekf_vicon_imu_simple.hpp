...
public:
    EstimatorViconImuTest(std::string yaml_file, std::string d_file)
    {
        ...
        error_base_quat_.resize(data_reader_.getNbRows(), 3);
    ...
    ///
    /// \brief run, simple method that execute the estimation of the base
    /// position from motion capture and IMU data saved from SL
    ///
    void run()
    {
        /* *** initialization *** */
        ...
        vicon_bse_->getBaseFromFilterState(output_base_position_,
                                           ...
                                           output_base_angular_velocity_);
        error_base_pose_.row(0) = (output_base_position_ -
                                   input_vicon_base_pose_.topRightCorner(3, 1))
                                      .transpose();
        error_base_quat_.row(0) =
            pinocchio::quaternion::log3(output_base_orientation_ *
                                        input_vicon_base_quat_.inverse())
                .transpose();
        ...
        for (int i = 1; i < data_reader_.getNbRows(); ++i)
        {
            ...
            get_data_from_file(i,
                               ...
                               input_vicon_base_quat_,
                               input_vicon_is_new_frame_);
            vicon_bse_->update(input_gyroscope_,
                               input_accelerometer_,
                               input_vicon_base_pose_,
                               input_vicon_is_new_frame_);
            vicon_bse_->get_filter_state(output_imu_position_,
            ...
            time_ += 0.002;
            data_collector_.updateDataCollection();
            error_base_pose_.row(i) =
                (output_base_position_ -
                 input_vicon_base_pose_.topRightCorner(3, 1))
                    .transpose();
            error_base_quat_.row(i) =
                pinocchio::quaternion::log3(output_base_orientation_ *
                                            input_vicon_base_quat_.inverse())
                    .transpose();
        }
    ...
    /// \param gyroscope: output imu data copied from the data set
    /// \param accelerometer: output imu data copied from the data set
    /// \param unfiltered_joint_data: joint position copied from the data set
    /// \param filtered_joint_data: filtered joint position copied from the data
    /// set \param vicon_base_pose: position of the based measured form the
    /// Vicon system copied from the data set \param vicon_is_new_frame: True is
    /// the Vicon data if actually a new one.
    ///
    void get_data_from_file(const int row,
                            Eigen::Vector3d& gyroscope,
                            Eigen::Vector3d& accelerometer,
                            Eigen::Matrix4d& vicon_base_pose,
                            Eigen::Quaterniond& input_vicon_base_quat,
                            bool& vicon_is_new_frame)
    {
        // get the ids of the data.
        data_reader_.fillVector(row, gyroscope_ids_, gyroscope);
        data_reader_.fillVector(row, accelerometer_ids_, accelerometer);
        Eigen::VectorXd vicon_base_pose_vec;
        vicon_base_pose_vec.resize(16);
        data_reader_.fillVector(row, vicon_base_pose_ids_, vicon_base_pose_vec);
        ...
        input_vicon_base_quat = vicon_base_pose.topLeftCorner<3, 3>();
    ...
    void subscribe_to_data_collector()
    {
        ...
        /* *** time_ *** */
        data_collector_.addVariable(&time_, "time", "-");
        // input
        /* *** input_imu_data_ *** */
        data_collector_.addVector3d(
            input_gyroscope_, "input_imu_gyro", "rad/s");
        data_collector_.addVector3d(
            input_accelerometer_, "input_imu_acc", "m/s^2");

        /* *** input_vicon *** */
        data_collector_.addMatrix(
            input_vicon_base_pose_, "input_vicon_base_pose", "-");
        data_collector_.addQuaternion(input_vicon_base_quat_,
                                      "input_vicon_base_quat");
        data_collector_.addVariable(
            &input_vicon_is_new_frame_, "input_vicon_is_new_frame", "-");
        ...
        /* *** input_vicon_base_pose_ *** */
        data_collector_.addMatrix(
            input_vicon_base_pose_, "vicon_hip_frame", "-");
        /* *** input_vicon_is_new_frame_ *** */
        ...
        data_collector_.addVector3d(
            output_imu_position_, "output_imu_position", "m");
        /* *** output_imu_orientation_ *** */
        data_collector_.addQuaternion(output_imu_orientation_,
                                      "output_imu_orientation");
        /* *** output_imu_linear_velocity_ *** */
        data_collector_.addVector3d(
            output_imu_linear_velocity_, "output_imu_linear_velocity", "m/s");
        /* *** output_imu_angular_velocty_ *** */
        data_collector_.addVector3d(output_imu_angular_velocty_,
                                    "output_imu_angular_velocity",
                                    "rad/s");
        /* *** output_base_position_ *** */
        data_collector_.addVector3d(
            output_base_position_, "output_base_position", "m");
        /* *** output_base_orientation_ *** */
        data_collector_.addQuaternion(output_base_orientation_,
                                      "output_base_quat");
        /* *** output_base_linear_velocity_ *** */
        data_collector_.addVector3d(
            output_base_linear_velocity_, "output_base_linear_velocity", "m/s");
        /* *** output_base_angular_velocty_ *** */
        data_collector_.addVector3d(output_base_angular_velocity_,
                                    "output_base_angular_velocity",
                                    "rad/s");
        /* *** output_accel_bias_ *** */
        data_collector_.addVector3d(
            output_accel_bias_, "output_accel_bias", "m/s^2");
        /* *** input_vicon_is_new_frame_ *** */
        data_collector_.addVector3d(
            output_gyro_bias_, "output_gyro_bias", "rad/s");
    }

    ///
    /// \brief get_data_indexes
    /// In order to parse the SL data file one need the indexes associated to
    /// the data. This method computes the different names and indexes.
    ///
    void get_data_indexes()
    {
        // create data stream names.
        std::vector<std::string> gyroscope_names(3), accelerometer_names(3),
        ...
        vicon_is_new_frame_id_ =
            data_reader_.getIndex(estimator_name + "is_new_frame");
    }

    void display_one_statistics(double avg,
                                double min,
                                double max,
                                std::string name)
    {
        std::cout << "average error in " << name << ": " << avg << std::endl;
        std::cout << "min absolute error in " << name << ": " << min
                  << std::endl;
        std::cout << "max absolute error in " << name << ": " << max
                  << std::endl;
        std::cout << std::endl;
    }
    ...
    void display_all_statistics()
    {
        std::cout << "########### Statistics ##########" << std::endl
                  << std::endl;
        display_one_statistics(avg_err_base_pose,
                               min_err_base_pose,
                               max_err_base_pose,
                               "BASE_POSE");
        display_one_statistics(avg_err_base_pose_x,
                               min_err_base_pose_x,
                               max_err_base_pose_x,
                               "BASE_POSE_X");
        display_one_statistics(avg_err_base_pose_y,
                               min_err_base_pose_y,
                               max_err_base_pose_y,
                               "BASE_POSE_Y");
        display_one_statistics(avg_err_base_pose_z,
                               min_err_base_pose_z,
                               max_err_base_pose_z,
                               "BASE_POSE_Z");
        display_one_statistics(avg_err_base_quat_x,
                               min_err_base_quat_x,
                               max_err_base_quat_x,
                               "BASE_QUAT_X");
        display_one_statistics(avg_err_base_quat_y,
                               min_err_base_quat_y,
                               max_err_base_quat_y,
                               "BASE_QUAT_Y");
        display_one_statistics(avg_err_base_quat_z,
                               min_err_base_quat_z,
                               max_err_base_quat_z,
                               "BASE_QUAT_Z");
        std::cout << "#################################" << std::endl
    ...
    {
        compute_one_statistics(error_base_pose_,
                               avg_err_base_pose,
                               min_err_base_pose,
                               max_err_base_pose);
        compute_one_statistics(error_base_pose_.col(0),
                               avg_err_base_pose_x,
                               min_err_base_pose_x,
                               max_err_base_pose_x);
        compute_one_statistics(error_base_pose_.col(1),
                               avg_err_base_pose_y,
                               min_err_base_pose_y,
                               max_err_base_pose_y);
        compute_one_statistics(error_base_pose_.col(2),
                               avg_err_base_pose_z,
                               min_err_base_pose_z,
                               max_err_base_pose_z);
        compute_one_statistics(error_base_quat_.col(0),
                               avg_err_base_quat_x,
                               min_err_base_quat_x,
                               max_err_base_quat_x);
        compute_one_statistics(error_base_quat_.col(1),
                               avg_err_base_quat_y,
                               min_err_base_quat_y,
                               max_err_base_quat_y);
        compute_one_statistics(error_base_quat_.col(2),
                               avg_err_base_quat_z,
                               min_err_base_quat_z,
                               max_err_base_quat_z);
    }
...
private:
    ...
    std::vector<int> gyroscope_ids_, accelerometer_ids_, vicon_base_pose_ids_;
