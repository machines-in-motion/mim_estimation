 ...
 *
 *  Using a taylor expansion of the previous equaiton we get:
 *  \f{align}
 *    x_t = g(u_t, \mu_{t-1}) + G_t \; * \; (x_{t-1} - \mu_{t-1}) + \epsilon_t \\
 *    z_t = h(\mu_t) + H_t \; * \; (x_{t} - \mu_{t}) + \delta_t
 *  \f}
 *  With \f$ \mu_{t} \f$ the average of the estimated state of covariance
 *  \f$ \Sigma_t \f$.
 *  The equation of the EKF routine are defined with the "update_filter" fonction.
 *
...
public:
   ...
   * @param numerical_jac: do we compute the Jacobians alanytically or not?
   ...
   * (see constructor). If the system is continuous we need to build a discrete
   * approximation of it. In order to do so we use the Van Loan's method
   * (see [2] slide 15).
   ...
   * - 2/ Obtain the matrix M expoential
   * \f{align}
   *  e^{M \Delta t} =
   *  \left[ \left.
   ...
   * \f}
   * - 4/ Compute the discretized process jacbian as follow:
   * \f{align}
   *  F_{t, disc} = R_{t, disc} \;\;\; e^{M \Delta t}[\text{upper right corner}]
   * \f}
   * In the following we will discard the \f$ {disc} \f$ subscript in the
   * expression of the process Jacobian and noise covariance for sake of clarity.
   * We assume that the system is either discrete or already discretized.
   *
   ...
   * \f$ \tilde{\Sigma}_{t} \f$ according to the model and the current control:
   * \f{align}
   ...
   * one occure.
   * \f{align}
   *    K_{t} &= \tilde{\Sigma}_{t} H_t^T (H_t \tilde{\Sigma}_t H_t^T + Q_t)^{-1} \\
 ...
 protected:
  ...
  S state_pre_;   /*!< Predicted state (\f$ \tilde{\mu} \f$, \f$ \tilde{\Sigma} \f$)*/
  ...
  /**
   * \brief processModel, a model of the continuous/discrete state dynamics \f$ g(u_t, x_{t-1}) \f$
   * \param s: \f$ [x_{t-1}^T \;\; u_t^T]^T \f$
   * \return the predicted state (Max: should not return anything and update state_pre_?)
  */
  virtual Eigen::Matrix<double,N,1> processModel(S &s) = 0;

  /**
   * @brief formProcessJacobian, compute the Jacobian of the process \f$ G_t \f$
   */
  virtual void formProcessJacobian(void) = 0;

  /**
   * @brief formProcessNoise, compute the covariance of the process noise \f$ R_t \f$
   */
  virtual void formProcessNoise(void) = 0;

  /**
   * @brief formNoiseJacobian, compute the Jacobian of the process noise \f$ E_t \f$
   */
   ...
   * @return the predicted measurement (Max: should not return anything and update meas_pred_?)
   */
  virtual Eigen::Matrix<double,K,1> measModel(S &s) = 0;

  /**
   * @brief formMeasJacobian, compute the Jacobian of the measurement \f$ H_t \f$
   */
   ...
   * @param[in, out] ds: the finite differentiation delta \f$ \frac{f(x+ds) - f(x-ds)}{2ds} \f$
   */
  ...
  Eigen::MatrixXd VL_mat_;      /*!< for Van Loan discretization, \f$ M \f$ */
  Eigen::MatrixXd exp_VL_mat_;  /*!< for Van Loan discretization, \f$ e^{M dt} \f$ */

  Eigen::MatrixXd proc_jac_disc_;     /*!< discretized/discrete process Jacobian, \f$ G_{t,disc}/G_t \f$ */
  Eigen::MatrixXd proc_jac_;          /*!< user input process Jacobian, \f$ G_t \f$ */
  Eigen::MatrixXd noise_jac_;         /*!< Jacobian of the process noise, \f$ E_t \f$ */
  Eigen::MatrixXd meas_jac_;          /*!< Jacobian of the measurement, \f$ H_t \f$ */
  Eigen::MatrixXd proc_noise_;        /*!< user input covariance of the process noise \f$ R_t \f$ */
  Eigen::MatrixXd proc_noise_disc_;   /*!< discretized/discrete covariance of the process noise \f$ R_{t,disc}/R_t \f$ */
  Eigen::MatrixXd meas_noise_;        /*!< user input covariance of the measure noise \f$ Q_t \f$ */
  Eigen::MatrixXd meas_noise_disc_;   /*!< covariance of the process noise \f$ Q_t \f$ assuming a discrete measurement */
  Eigen::MatrixXd gain_mat_;          /*!< Kalman filter gain matrix \f$ K_t \f$ */

  Eigen::LDLT<Eigen::MatrixXd> ldlt;  /*!< LDLT decomposition to invert some matrix */

  bool numerical_jac_;                /*!< Do we compute the Jacobian
                                           numericaly or analyticaly? */
  bool is_discrete_;                  /*!< Is the process discrete? */
