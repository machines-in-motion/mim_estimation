...
/*!
 *  Specialization to an Eigen::Vector. This Filter class implements a
 * butterworth low pass filter able to filter a vector of measurements. Only
 * Eigen::Vectors or Eigen::Matrices with column dimension 1 are valid. Usage
 * example: typedef Filter<Eigen::Matrix<double,6,1> > FilterType; INSTANTIATE
 * FilterType myfilter; INITIALIZE       myfilter.initialize(initial_state,
 * filter_order, cuttoff_frequency); UPDATE myfilter.update(measurement);
...
public:
    ...
    typedef Eigen::Matrix<_Scalar, _Rows, 1, Eigen::ColMajor, _Rows, 1>
        VecToFilter;
...
public:
     ...
     *  @param[in] current_state	initial state of the system of type
     * VecToFilter
     *  @param[in] filter_order	order of the butterworth filter (integer >= 1)
     *  @param[in] cutoff_frequency	cuttoff frequency for the filter as
     * percentage of the nyquist frequency. For example, if sampling frequency
     * is 200Hz; then the Nyquist frequency is half of the sampling frequency,
     * namely 100Hz. cut_off frequency is a double element of [0-1], such that 0
     * corresponds to 0*Nyquist frequency and 1 corresponds to a
     * cutoff_frequency equal to the Nyquist frequency.
     */
     ...
     *  @param[out] current_state	filtered estimate of state of type
     * VecToFilter
     */
...
private:
    /*!
     *  Function to compute the coefficients of a Butterworth filter without
     * reading them from a table. It only needs to be done once at
     * initialization. return                        true if successfull
    ...
    //! cutoff frequency for the butterworth filter ([0-1] percentage of the
    //! nyquist frequency)
    double cutoff_;
