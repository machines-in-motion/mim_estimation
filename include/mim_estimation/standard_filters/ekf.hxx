...
EKF<S, M>::EKF(bool numerical_jac,
               bool is_discrete,
               double dt,
               unsigned lin_order)
...
void EKF<S, M>::updateFilter(bool update_on)
{
    ...
    if (update_on)
    {
        ...
        state_post_.state_cov =
            state_pre_.state_cov - gain_mat_ * meas_jac_ * state_pre_.state_cov;
    }
    else
    {
...
bool EKF<S, M>::check_jacobians(bool evaluate)
{
    ...
    if (evaluate)
    {
        ...
        double error_proc_jac =
            (proc_jac_fd - Eigen::MatrixXd::Identity(N, N) - dt_ * proc_jac_)
                .norm();
